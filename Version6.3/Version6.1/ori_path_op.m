function [ PathStore_MAJ_res,Path_num_MAJ_res,D] = ori_path_op(D_ori,D,X_start,Y_start,X_fin,Y_fin,PathStore,Path_num,SD,i,encarde,res)
    Goal_op_x=X_fin-X_start+encarde+1; % 终点坐标转换
    Goal_op_y=Y_fin-Y_start+encarde+1;
    Goal_op = Goal_op_y+(Goal_op_x-1)*(2*encarde+1);
    temp_D=D(X_start-encarde:X_start+encarde,Y_start-encarde:Y_start+encarde);
    D_reduit=D_ori(X_start-encarde:X_start+encarde,Y_start-encarde:Y_start+encarde);
    ...
    [RE,PATH,~]=Modify_path(temp_D,central,Goal_op,i);
      ...
      if RE == 1
          ...
          if flgn==1
                ...
                [PATH_sup,~]=sup_path(temp_D,D_reduit,Goal_op+m,Goal_op,SD_temp,i)  ;
                PATH=[PATH;PATH_sup];
