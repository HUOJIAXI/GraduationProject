...
for i = 1:RobotNum
    ...
    while PathStore{i}(count) ~= Goal(i)
        ...
        if ~((temp_x==1&&temp_y==1) && (temp_x==1&&temp_y==size_n)&&(temp_x==size_m&&temp_y==size_n)&&(temp_x==size_m&&temp_y==1))
...
%         elseif (temp_x==1&&temp_y~=1)||(temp_x==size_m&&temp_y~=size_n)
%             if ini_dir_way( PathStore{i}(count)) ==1 %若所在巷道为从西到东的方向或从北到南          
%                     PathStore{i}=[PathStore{i}, PathStore{i}(count)+1];
%                     count=count+1;
% 
