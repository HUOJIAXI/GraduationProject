function [PathStore_MAJ_res,Path_num_MAJ_res,Start,Goal,temp]=op_modify_path(temp,X_start,Y_start,X_fin,Y_fin,Start,Goal,Path_num,PathStore,j,res,SD)
...
% if (Y_fin-Y_start)*(Y_fin-Y_start)+ (X_fin-X_start)*(X_fin-X_start)> (encarde+1)*(encarde+1)*2
 ...
 %if shorest_path > (encarde+1)*(encarde+1)*2
     ...
     if temp_reduit(Goal_X_fin,Goal_Y_fin) == 1
        ...
        disp('缩减图开始求解')
        [~,PATH,~]=Modify_path(temp_reduit,centrale,Goal_res,j);  % 第j个冲突机器人路径重新规划，将规划得到的路径替换掉在原始路径中对应的部分。
